...
expected_test_dicts = [
    test_dict
    for test_dict in common_nn.module_tests + \
        common_nn.new_module_tests + \
        common_nn.criterion_tests + \
        common_nn.new_criterion_tests + \
        sample_module.module_tests + \
        sample_functional.functional_tests
    if test_dict.get('test_cpp_api_parity', True)
]

# Assert that we've covered all tests
assert len([name for name in TestCppApiParity.__dict__ if 'test_torch_nn_' in name]) == \
    len(expected_test_dicts) * len(devices)
